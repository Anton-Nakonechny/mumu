...
1. **Given** the app is in Learn mode in Ukrainian, **When** the user taps the Spanish flag, **Then** all on-screen text switches to Spanish and the next spoken phrase is in Spanish.
2. **Given** the app is in Learn mode in Spanish, **When** the user taps the English flag, **Then** all on-screen text switches to English and the next spoken phrase is in English.
3. **Given** any language is active, **When** the user taps the currently selected flag, **Then** nothing changes (no flicker or reset).
4. **Given** the app is in Quiz mode and listening for a voice answer, **When** the user taps a different flag, **Then** listening stops, language switches, and the quiz prompt is presented in the new language.
...
- What if the app is mid-animation or mid-cheer when the language is switched? The ongoing audio finishes; subsequent interactions use the new language.
...
- **FR-004**: System MUST switch ALL spoken audio (learn phrases, quiz prompts, cheer responses, sound reveals) to the selected language immediately when a flag is tapped.
...
- **SC-002**: All interface text and spoken audio update to the newly selected language within 1 second of tapping a flag.
