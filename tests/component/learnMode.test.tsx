...
describe('LearnMode (User Story 1)', () => {
  it('speaks the learn phrase on mount', () => {
    const tts = makeTts();
    render(<LearnMode animal={cow} tts={tts} onNext={vi.fn()} onPrev={vi.fn()} />);
    expect(tts.speak).toHaveBeenCalledWith('The cow says muuuu');
  ...
  it('speaks again when the animal changes', () => {
    ...
    const { rerender } = render(
      <LearnMode animal={cow} tts={tts} onNext={vi.fn()} onPrev={vi.fn()} />,
    );
    rerender(<LearnMode animal={dog} tts={tts} onNext={vi.fn()} onPrev={vi.fn()} />);
  ...
  it('cancels speech and calls onNext when navigating', () => {
    const tts = makeTts();
    const onNext = vi.fn();
    render(<LearnMode animal={cow} tts={tts} onNext={onNext} onPrev={vi.fn()} />);
